...
def check_cropping(samples,image,name,spatial_pos_csv,barcode_csv,res,root_path):
    ...
    for each, i in zip(barcode_csv,range(len(barcode_csv))):
        ...
        if check(cropped_image,0.75):
            ...
            patch_image.save(parent_dir+name+"/"+each+".png") # saving the spot image into Sample folder. Create this directory of Imags/Sample_id
    ...
    with torch.autocast(device_type="cuda", dtype=torch.float16):
        with torch.inference_mode():
            for name in samples:
                ...
                count = 0
                barcodes = \
                pd.read_csv(root_path + "filtered_barcodes/" + name + "_sample_filtered_barcodes_1.tsv", sep="\t", header=None)[0].values
                for bar in barcodes:
...
def main():
    ...
    pathway_dict = ov.utils.geneset_prepare(cfg.pathway_dict_file) # replace with required pathway database, i.e., GO/KEGG/MsigDB
    uni_pathway_genes=[]
    ...
    print("Saving the pathway-associated unique common genes in filtered spots to a numpy array")
    ...
    print("Saving the filtered pathway dic in JSON....")
